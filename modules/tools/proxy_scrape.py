        ...
        match option:
            case "1":
                ...
                if not len(original_links):
                    ...
                    return
                print(
                    f"    [{cyan}>{reset}] Imported {green}{len(original_links)}{reset} Links"
                )
                if duplicates != 0:
                    print(
                        f"    [{cyan}>{reset}] Removed {green}{duplicates}{reset} Duplicates"
                    )
                sleep(0.5)
                scrape(after_links)
            case "2":
...
def scrape(links: list = None):
    ...
    def get_proxies(link):
        ...
        try:
            ...
            log(None, f"Connection Error ~ {link}")
        else:
            for line in request:
                try:
                    if ":" in line:
    ...
    with open(
        f"results/{tree_name}/{tree_name}_proxies.txt", "w", errors="ignore"
    ) as file:
        file.write("\n".join(proxies))
    save_path = os.path.join(os.getcwd(), f"results/{tree_name}")
    print(f"\n\n    [{cyan}>{reset}] Finished Scraping")
    print(f'    [{cyan}>{reset}] Saved to "{save_path}/{tree_name}_proxies.txt"')
