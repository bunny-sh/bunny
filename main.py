...
def info():
    ...
    print(
        f"""
    [{cyan}>{reset}] Created By: {discord_name}
    [{cyan}>{reset}] Discord Server: {discord_server}
        ...
        if option.isdigit():
            if int(option) <= len(modules_list) and int(option):
                ...
                selected_modules.append(
                    module
                ) if module not in selected_modules else selected_modules.remove(module)

        elif "," in option:
            for module in option.split(","):
                if (
                    module.isdigit()
                    and int(module) <= len(modules_list)
                    and int(module)
                ):
                    module = list(modules_list)[int(module) - 1]
                    selected_modules.append(
                        module
                    ) if module not in selected_modules else selected_modules.remove(
                        module
                    )
...
def settings():
    while 1:
        load_config()
        change_title(f"bunny.sh | Settings | {discord_name}")
        clear()
        ascii()
        if Checker.api_key:
    ...
    [{cyan}X{reset}] Back"""
        ...
        match option:
            ...
            case "5":
                change("threads")
            case "6":
                change("solver_service")
            case "7":
                change("api_key")
            case "8":
                change("discord_webhook")
            case "o":
                os.startfile(os.path.join(os.getcwd(), "data/config.json"))
            case "x":
                return
...
if __name__ == "__main__":
    ...
    input(
        f"""
    [{red}Creator Info{reset}]
    [{green}Website{reset}] https://bunny.giotje.dev
    [{dark_cyan}Github{reset}] https://github.com/bunny-sh
    [{dark_yellow}Continue{reset}] Enter
"""
    )
